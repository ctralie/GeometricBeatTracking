...
def doRipsFiltrationDM(D, maxHomDim, thresh = -1, coeff = 2, getCocycles = False, relpath = "./"):
    """
    ...
    if thresh > 0:
        callThresh = thresh
    if getCocycles:
        proc = subprocess.Popen(["%sripser/ripser-representatives"%relpath, "--format", "distance", "--dim", "%i"%maxHomDim, "--threshold", "%g"%callThresh, "--modulus", "%i"%coeff, "D.txt"], stdout=subprocess.PIPE)
    elif coeff > 2:
        proc = subprocess.Popen(["%sripser/ripser-coeff"%relpath, "--dim", "%i"%maxHomDim, "--threshold", "%g"%callThresh, "--modulus", "%i"%coeff, "D.txt"], stdout=subprocess.PIPE)
    else:
        proc = subprocess.Popen(["%sripser/ripser"%relpath, "--dim", "%i"%maxHomDim, "--threshold", "%g"%callThresh, "D.txt"], stdout=subprocess.PIPE)
...
def doRipsFiltration(X, maxHomDim, thresh = -1, coeff = 2, getCocycles = False, relpath = "./"):
    """
    ...
    """
    D = getSSM(X)
    return doRipsFiltrationDM(D, maxHomDim, thresh, coeff, getCocycles, relpath)
